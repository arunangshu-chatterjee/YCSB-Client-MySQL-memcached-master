...
public class JdbcDBClient extends DB {
	...
	private static boolean batchUpdates;
	...
	public static boolean stringkey;
	...
	public void init() throws DBException {
		...
		stringkey = Boolean.parseBoolean(props.getProperty("stringkey", "true"));
		...
		try {
			...
			System.out.println(
					"Using shards: " + shardCount + ", batchSize:" + batchSize + ", fetchSize: " + jdbcFetchSize);
			...
			System.err.println("Error in database operation: " + e + " " + driver);
			throw new DBException(e);
		...
		if (batchSize > 0) {
			try {
				...
				for (PreparedStatement st : cachedStatements.values()) {
					if ((numRowsInBatch % batchSize != 0) && !st.getConnection().isClosed() && !st.isClosed()) {
						st.executeBatch();
					}
				}
			} catch (SQLException e) {
	...
	public Status read(String tableName, String key, Set<String> fields, HashMap<String, ByteIterator> result) {
		try {
			...
			if (stringkey)
				...
				readStatement.setLong(1, Long.parseLong(key));
			...
			if (result != null) {
				...
				if (fields != null) {
					...
					for (int i = 1; i <= columnCount; i++) {
						...
						if (value != null) {
							result.put(md.getColumnLabel(i), new StringByteIterator(value));
						}
					}
				}
			}
	...
	public Status scan(String tableName, String startKey, int recordcount, Set<String> fields,
		...
		try {
			...
			if (stringkey)
				...
				scanStatement.setLong(1, Long.parseLong(startKey));
			...
			for (int i = 0; i < recordcount && resultSet.next(); i++) {
				if (result != null && fields != null) {
					...
					values.put(PRIMARY_KEY, new StringByteIterator(value));
					for (String field : fields) {
						value = resultSet.getString(field);
						values.put(field, new StringByteIterator(value));
					}
					result.add(values);
				} else if (fields == null && result != null) {
					HashMap<String, ByteIterator> values = new HashMap<String, ByteIterator>();
					ResultSetMetaData md = resultSet.getMetaData();
					int columnCount = md.getColumnCount();
					...
					for (int j = 1; j <= columnCount; j++) {
						...
						values.put(md.getColumnLabel(j), new StringByteIterator(value));
					}
					result.add(values);
	...
	public Status update(String tableName, String key, HashMap<String, ByteIterator> values) {
		try {
			...
			StatementType type = new StatementType(StatementType.Type.UPDATE, tableName, numFields,
					fieldInfo.getFieldKeys(), getShardIndexByKey(key));
			...
			if (stringkey)
				...
				updateStatement.setLong(index, Long.parseLong(key));
	...
	public Status insert(String tableName, String key, HashMap<String, ByteIterator> values) {
		try {
			...
			if (stringkey)
				...
				insertStatement.setLong(1, Long.parseLong(key));
			int index = 2;
			for (String value : fieldInfo.getFieldValues()) {
	...
	public Status delete(String tableName, String key) {
		try {
			...
			if (stringkey)
				...
				deleteStatement.setLong(1, Long.parseLong(key));
